...
const UploadBanner: React.FC<UploadBannerProps> = ({ pickedAsset, onAsset }) => {
	...
	let content = (
		<SvgBanner Svg={UploadSvg} placeholder="Click to upload a new post" textStyle={styles.placeholderText} />
	);
		...
		content = (
			<>
				<IconButton
					...
					Icon={() => <AntDesign name="closecircle" color={ThemeStatic.white} size={IconSizes.x6} />}
				/>
	...
	return (
		<TouchableOpacity
			activeOpacity={0.95}
			onPress={pickImage}
			style={[styles.container, { borderWidth: pickedAsset ? 0 : StyleSheet.hairlineWidth }]}>
			{content}
		</TouchableOpacity>
	);
};
